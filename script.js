...
async function fetchNews(category) {
  try {
    let res = await fetch(
      `${URL}${category}&from=2024-11-16&apiKey=${API_KEY}`
    );
...
const bindNews = (articles) => {
  ...
  if (articles.length > 0) {
    let str = "";
    articles
      .filter((article) => article.urlToImage && article.urlToImage !== "null")
      .forEach((article) => {
        str += `
            <div class="col-xl-4 news-card">
                <div class="card">
                    <img src="${
                      article.urlToImage
                    }" class="card-img-top" alt="news-img" />
                    <div class="card-body">
                        <h4 class="card-title">${article.title}</h4>
                        <h6>${article.source.name} 📢 ${new Date(
          article.publishedAt
